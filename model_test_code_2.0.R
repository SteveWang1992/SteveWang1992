...
pred_result <- parLapply(cl, model_lst, model_pred)
pred_result_check <- sapply(pred_result, length)

if (all(pred_result_check == 0)) {
    pred_result <- parLapply(cl, model_lst, model_pred, func = "bigram")
    pred_result_check <- for (i in pred_result) length(i)
    if (all(pred_result_check == 0)) {
        pred_result <- parLapply(cl, model_lst, model_pred, func = "unigram")
    }
}
...
test <- do.call(bind_rows, pred_result)
test[is.na(test)] <- 0
test %>% 
    summarise_all(sum) %>% 
    gather(key = "var", value = "number") %>% 
